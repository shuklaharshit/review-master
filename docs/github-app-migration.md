...
> **App registered** with decisions F-1=ON (refresh tokens) and F-3="Any account". No private key generated (not needed — device flow / user-to-server only). The Client ID and slug are **public, not secrets**, so they're **hardcoded in `src/shared/constants.ts`** as the canonical `review-master-ai` App (a clone works out of the box). A fork points at its own App by editing those two constants.
